					...
					<table class="table table-striped table-bordered table-hover">
						...
						<tbody>
		                    ...
		                    <tr>
								...
								<td class="text-center">${list.playerId}</td>
								<td class="text-center">${list.name}</td>
								<td class="text-center">${list.backNumber}</td>
								<td class="text-center">${list.position}</td>
								<td class="text-center">
								<c:if test="${list.throwing eq 'L'}">${list.throwing = '좌'}</c:if>
								<c:if test="${list.throwing eq 'R'}">${list.throwing = '우'}</c:if>
								투
								<c:if test="${list.hitting eq 'L'}">${list.hitting = '좌'}</c:if>
								<c:if test="${list.hitting eq 'R'}">${list.hitting = '우'}</c:if>
								타</td>
								<td class="text-center"><a href="${ctx}/teamDetail.do?id=${list.teamId}">${team.name}</a></td>
								<td class="text-center"><a href="${ctx}/tradeTargetList.do?id=${list.playerId}">트레이드</a></td>
							</tr>
							</c:forEach>
							</c:forEach>
							</c:if>
